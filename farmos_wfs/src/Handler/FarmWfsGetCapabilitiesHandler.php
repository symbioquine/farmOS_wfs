...
class FarmWfsGetCapabilitiesHandler {
   ...
   *          The current user.
   */
  public function __construct(RequestStack $request_stack, ConfigFactoryInterface $config_factory, AccountProxyInterface $currentUser) {
  ...
  public function handle(array $query_params) {
    ...
    return farmos_wfs_makeDoc(function ($doc, $elem) use ($host) {
      $doc->appendChild($elem('wfs:WFS_Capabilities', array(
        ...
        $wfsCapabilities->appendChild($elem('ows:ServiceIdentification', [], function ($serviceIdentification, $elem) {
          ...
          $serviceIdentification->appendChild($elem('ows:Keywords', [], function ($keywords, $elem) {
        ...
        $wfsCapabilities->appendChild($elem('ows:ServiceProvider', [], function ($serviceProvider, $elem) use ($host) {

          $serviceProvider->appendChild($elem('ows:ProviderName', [], Xss::filterAdmin($this->configFactory->get('system.site')
            ->get('name'))));
        ...
        $wfsCapabilities->appendChild($elem('ows:OperationsMetadata', [], function ($operationsMetadata, $elem) use ($host) {
          ...
          foreach ([
            ...
            $operationsMetadata->appendChild($elem('ows:Operation', array(
              ...
              $operation->appendChild($elem('ows:DCP', [], function ($dcp, $elem) use ($operationName, $host) {

                $dcp->appendChild($elem('ows:HTTP', array(), function ($http, $elem) use ($operationName, $host) {
        ...
        $wfsCapabilities->appendChild($elem('wfs:FeatureTypeList', [], function ($featureTypeList, $elem) {

          $featureTypeList->appendChild($elem('wfs:Operations', [], function ($operations, $elem) {
          ...
          foreach (FARMOS_WFS_QUALIFIED_TYPE_NAMES as $type_name) {

            $featureTypeList->appendChild($elem('wfs:FeatureType', [], function ($featureType, $elem) use ($type_name) {
              ...
              $featureType->appendChild($elem('wfs:OutputFormats', [], function ($outputFormats, $elem) {
        ...
        $wfsCapabilities->appendChild($elem('ogc:Filter_Capabilities', [], function ($filterCapabilities, $elem) {

          $filterCapabilities->appendChild($elem('ogc:Spatial_Capabilities', [], function ($spatialCapabilities, $elem) {

            $spatialCapabilities->appendChild($elem('ogc:SpatialOperators', [], function ($spatialOperators, $elem) {
